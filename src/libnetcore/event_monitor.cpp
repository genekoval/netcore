...
namespace netcore {
        ...
        max_events(max_events)
    {
        ...
        TRACE()
            << epsock << " event monitor created with "
            << max_events << " max events";
    }

    auto event_monitor::add(int fd) -> void {
        ev.data.fd = fd;
        if (epoll_ctl(epsock.fd(), EPOLL_CTL_ADD, fd, &ev) == -1) {
    ...
    auto event_monitor::add(const socket& sock) -> void { add(sock.fd()); }

    auto event_monitor::set(uint32_t event_types) -> void {
        ev.events = event_types;
    }

    auto event_monitor::wait(const event_handler& callback) -> void {
